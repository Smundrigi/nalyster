...
public class BatchEditWindow extends JFrame {
     ...
     */
    public BatchEditWindow(Tab tab) {
        initComponents();
...
//        jString.setText("");
    ...
    private void setKeyBoardFocusManager() {
        ...
        /*Author ;swapna
            Date 27th Sept 2017
            Temporary table variable declared*/
        ...
        if (focusFrame.getFocusOwner() != this) {
            ...
            date: 28th Sept 2017
            */
            focusFrame.clearGlobalFocusOwner();
            this.requestFocus();
        }
            ...
            public boolean dispatchKeyEvent(KeyEvent e) 
            {
                ...
                else if (e.getKeyCode() == KeyEvent.VK_ENTER) 
                {
                   // projectManagerWindow.makeTableEditable(true);
                    ...
                    String columnName = comboBoxFieldSelect.getSelectedItem().toString();      // column name   
                    String newValue = textFieldNewValue.getText();                             // new value to replace old value(s)
                    int[] rows = tempTable.getSelectedRows();                                      // get selected rows
                    int columnIndex;                                                           // column index
                    int rowIndex;                                                              // row index
                    int rowCount = tempTable.getSelectedRowCount();                                // number of rows

                    // get column index for the combobox selection
                    for (columnIndex = 0; columnIndex < tempTable.getColumnCount(); columnIndex++) 
                    {
                        if (columnName.equals(tempTable.getColumnName(columnIndex))) {
                            break;
                        }
                    }
                    ...
                    // this also envokes the TableModelListener 
                    // and adds the change to the mofified data list for the table
                    JTable tempTable1 = tempTable;
                    for (rowIndex = 0; rowIndex < rowCount; rowIndex++) 
                    {
                        tempTable1.setValueAt(newValue, rows[rowIndex], columnIndex);
                    }
    ...
    private void closeWindow(){
        this.dispose();
        projectManagerWindow.setEnabled(true);
    }
